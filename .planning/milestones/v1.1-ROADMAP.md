...
- **Full Claude Code hook lifecycle.** All 6 hooks (SessionStart, SessionResume, UserPromptSubmit, PostToolUse, Notification, SessionEnd) with fire-and-forget queue pattern. SessionStart injects relevant past memories via `additionalContext` — context continuity across sessions. PostToolUse captures real-time tool signals. Progressive disclosure MCP search (3-layer: index → timeline → full entity). Web viewer delivered in Phase 11.
...
**Full Claude Code hook lifecycle** (Phase 14) — all 6 hook events observed with fire-and-forget queue pattern. SessionStart injects relevant memories at session open via `additionalContext` JSON output — the core value: Claude Code arrives with context about past decisions, patterns, and learnings without manual prompting. PostToolUse captures tool signals in real time. UserPromptSubmit, SessionResume, Notification, and SessionEnd round out the lifecycle. Non-blocking; hooks must return in <100ms.

**Local observation storage + Ollama summarization** (Phase 14) — tool observations compressed into structured summaries by local Ollama (no external API). Stored in a backend-agnostic memory layer (uses the DB backend chosen in Phase 12 — must not use Kuzu-specific FTS/HNSW APIs). Deduplication by session_id + content_hash.

**Progressive disclosure MCP search** (Phase 14) — 3-layer pattern:
1. `memory_search` → compact index (~50–100 tokens) — keyword FTS
2. `memory_timeline` → chronological context window around a result
3. `memory_details` → full observation detail (~500–1000 tokens) — fetched by ID only

**Web viewer** — `graphiti ui` launches a localhost browser view of the live knowledge graph. Delivered in Phase 11. ✓
...
**Requirements**: MEM-01 (6-hook lifecycle), MEM-02 (observation store + Ollama summarization), MEM-03 (3-layer MCP search), MEM-04 (SessionStart context injection), MEM-05 (integration tests)
**Success Criteria** (what must be TRUE):
...
**Source plans:** Adapted from `10-local-memory` ghost phase (planned 2026-03-xx, never wired into roadmap). Plan 02 implementation must be rewritten for the DB backend chosen in Phase 12.
**Plans:** 5 plans
