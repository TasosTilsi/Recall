...
Confirm `ui/out/index.html` exists after build. The `ui/out/` directory is committed to git as the pre-built static artifact — no Node.js needed at install time.
  ...
  <verify>
    <automated>cd /home/tasostilsi/Development/Projects/graphiti-knowledge-graph/ui && npm run build 2>&1; echo "exit=$?"</automated>
  </verify>
  <done>`npm run build` exits 0 (exit=0 in output), `ui/out/index.html` exists — TypeScript errors, SSR window crashes, and Tailwind import failures will surface in the build output if present</done>
</task>
